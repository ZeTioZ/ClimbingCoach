...
def get_user_route_records(app_state: AppState) -> list[Run]:
...
def get_all_users_route_records(app_state: AppState) -> list[Run]:
	"""Return all the route records."""
	selected_route = app_state.get_route()
	all_route_records = run_queries.get_runs_by_route(selected_route.name)
	all_route_records.sort(key=lambda x: x.runtime)
	return all_route_records
