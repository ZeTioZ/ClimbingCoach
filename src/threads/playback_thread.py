...
class Playback(Thread):
	def __init__(self, image: np.ndarray, frame_rate: int, skeletons_list: list, parent_page : Page, runtime: int = 0):
		super().__init__()
		self.daemon = True
	...
	def run(self):
		skeletons_index = self.get_current_progress()
		self.label_img.after(10, self.frame_draw_loop, skeletons_index)
	...
	def get_size_img(self, img : np.ndarray):
		...
		heigh = self.parent_page.app.winfo_height()
