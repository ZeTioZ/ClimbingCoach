...
class trail_page(page):
    ...
    def __init__(self, parent: customtkinter.CTkFrame, app: customtkinter.CTk):
        ...
        self.trail_image = customtkinter.CTkImage(Image.open(self.__get_trail_image_path(current_trail["image"])), size=(UV(200), UV(200)))
        ...
        self.trail_selection_button = customtkinter.CTkButton(self.trail_detail_frame,text="Select", command=lambda : self.selection_trail())
        self.trail_selection_button.grid(row=RID_DIFFICULTY, column=CID_RIGHT, sticky="n")

        
        self.detail_title_name = customtkinter.CTkLabel(self.trail_detail_frame, text=current_trail["name"], font=(FONT, IUV(32), "bold"), fg_color=SECONDARY_COLOR, corner_radius=20, width=UV(350))
        self.detail_title_name.grid(row=RID_TITLE, column=CID_LEFT, columnspan=2, sticky="", ipady=UV(10))

        self.detail_description = customtkinter.CTkTextbox(self.trail_detail_frame, font=(FONT, IUV(16)), wrap=tk.WORD, fg_color="transparent")
        ...
        for i in range(5):
            self.difficulty.append(customtkinter.CTkFrame(self.detail_difficulty, fg_color="green", corner_radius=UV(1000), width=UV(25), height=UV(25), border_color="white"))
